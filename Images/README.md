# 📸 Project Screenshots

This folder contains the screenshots used in the SAP HANA to Power BI End-to-End Sales Analytics Project.

---

## 🏗 Architecture Diagram

**File:** `Architecture Diagram.png`

Shows the complete end-to-end project architecture from CSV dataset to Power BI dashboard.

---

## 🖥 SAP HANA Environment

**File:** `Oracle Virtualbox.png`

Shows Oracle VirtualBox running Ubuntu Linux Server with SAP HANA Express Edition installed and operational.

---

## 📥 CSV Dataset Import

**File:** `SAP HANA DATA IMPORT.png`

Shows the process of importing the CSV sales dataset into SAP HANA Studio.

---

## 🗄 SAP HANA Sales Table

**File:** `SAP HANA TABLE.png`

Shows the imported sales records successfully stored inside the SAP HANA database.

---

## 🔗 Power BI Connection

**File:** `POWER BI CONNECTOR.png`

Shows Microsoft Power BI connecting to SAP HANA using the native SAP HANA Connector.

---

## 📊 Interactive Dashboard

**File:** `Dashboard.png`

Shows the final interactive sales analytics dashboard created in Microsoft Power BI.

---

These screenshots demonstrate the complete implementation of the project from data import to business intelligence reporting.
